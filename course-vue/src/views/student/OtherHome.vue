<template>
    <div class="homepage">
        ...
        <div class="GPA">
            ...
            <div id="hisChart" ref="EChart" style="width: 400px; height: 300px;" class="gpaChart"></div>
        </div>
        <div class="score">
            <p class="title">分数区间</p>
            <div id="hisScoreChart" ref="SChart" style="width: 400px; height: 250px;" class="scoreChart"></div>
        </div>
...
const updateTableData = async () => {
    const info = await request.post('/student/getStudentIntroduceDataByStudentId',{
        data:{
...
const gpa = ref()
const getRenderer =  () => {
    const dom1 = document.getElementById('hisChart');
    const dom2 = document.getElementById('hisScoreChart');
      const hisChart = echarts.init(dom1); // 初始化echarts实例
      const hisScoreChart = echarts.init(dom2);
      const option1 = {
        series: [{
            color:["#6FB6C1","#e6e6e6"],
            type: 'pie',
