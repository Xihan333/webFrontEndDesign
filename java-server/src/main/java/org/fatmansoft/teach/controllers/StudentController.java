...
import org.fatmansoft.teach.service.creativePractice.ScientificPayoffsService;
...
public class StudentController {
    ...
    @PostMapping("/getStudentOptionItemListByCourseIdAndHomeworkName")
    public OptionItemList getCourseOptionItemListByCourseIdAndHomeworkName(@Valid @RequestBody DataRequest dataRequest) {
        Integer courseId=dataRequest.getInteger("courseId");
        String homeworkName=dataRequest.getString("homeworkName");
        List<Student> sList = studentRepository.findStudentListByCourseIdAndHomeworkName(courseId,homeworkName);  //数据库查询操作
        List<OptionItem> itemList = new ArrayList();
        for (Student s : sList) {
            itemList.add(new OptionItem(s.getStudentId(), s.getPerson().getNum(), s.getPerson().getNum()+"-"+s.getPerson().getName()));
        }
    ...
    @PostMapping("/getStudentOptionItemListByCourseIdAndDate")
    public OptionItemList getCourseOptionItemListByCourseIdAndDate(@Valid @RequestBody DataRequest dataRequest) {
        Integer courseId=dataRequest.getInteger("courseId");
        String date=dataRequest.getString("date");
        List<Student> sList = studentRepository.findStudentListByCourseIdAndDate(courseId,date);  //数据库查询操作
        List<OptionItem> itemList = new ArrayList();
        for (Student s : sList) {
            itemList.add(new OptionItem(s.getStudentId(), s.getPerson().getNum(), s.getPerson().getNum()+"-"+s.getPerson().getName()));
        }
    ...
    public DataResponse getStudentIntroduceData(@Valid @RequestBody DataRequest dataRequest) {
        ...
        data.put("info",info);
        data.put("familyList",familyService.getFamilyMapListByStudentId(s.getStudentId()));
        data.put("educationExperienceList",educationExperienceService.getStudentEducationExperienceMapList((s.getStudentId())));
        data.put("achievementList",achievementService.getPassedAchievementMapList(s.getPerson().getNum()));
        data.put("punishmentList",punishmentService.getPunishmentMapList(s.getPerson().getNum()));
        data.put("competitionList",competitionService.getCompetitionMapList(s.getPerson().getNum()));
        data.put("newProjectList",newProjectService.getNewProjectMapList(s.getPerson().getNum()));
        ...
        data.put("feeList",studentService.getStudentFeeList(s.getStudentId()));
